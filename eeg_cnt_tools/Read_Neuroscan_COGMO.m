%% This program opens and reads a CNT file from Neuroscan
function [f,lab,dat,labels_electrodes,events_trigger,events_type] = Read_Neuroscan_COGMO(CNT_file_selected,bits_data,CNT_file_directory)

% cd(CNT_file_directory)
CNT_file_path = fullfile(CNT_file_directory, CNT_file_selected);
...
fid = fopen(CNT_file_path,'r','l');    
disp(['Opening and reading the neuroscan file ' CNT_file_path ' ...'])
%'r'     open file for reading
%'l' - IEEE floating point with little-endian byte ordering

%% Starting reading the information from the header file
%  "h" will contain the information which pertains to all channels (i.e. digitization rate, pre-trigger interval, etc.)
...
h.nchannels         = fread(fid,1,'ushort');    %Number of channels recorded
...
h.eventtablepos     = fread(fid,1,'ulong'); %Length of the "cnt" file
...
%% Reading the information for each of the "n.channels" channels
%  "e" will contain the information which pertains to particular channels (i.e. electrode label, calibration factor, etc.)

for n = 1:h.nchannels
    e(n).lab            = deblank(char(fread(fid,10,'char')')); %Name of electrode 
...
% ----------------------------------
%% "begdata = ftell(fid)" Returns the location of the file position indicator for the file specified 
%  by fid; begdata will take the value "6000" for 68 channels, as the header is 900samples, while the size of the
%  channel-specific part of the header is 75 samples per channel (75*68 = 5100 samples)

begdata = ftell(fid);   
enddata = h.eventtablepos;   %"h.eventtablepos" is the last sample of the last channel recorded;
if strcmpi(r.dataformat, 'int16')
     nums    = (enddata-begdata)/h.nchannels/2;
else nums    = (enddata-begdata)/h.nchannels/4; %Returns the number of samples per channel 
    %(I am not sure why we have to divide by 4)
end;
...
else startpos = r.t1*h.rate*4*h.nchannels;  %Position where to start reading the data; 
    %since "fread" has already shifted the data by 6000 points, the starting position will be "0" 
    %(no need for any additional shift) 
end;
if isempty(r.ldnsamples)
     if ~isempty(r.lddur)
          r.ldnsamples = round(r.lddur*h.rate); 
     else r.ldnsamples = nums; %Samples for each channel
     end;
end;
...
if type == 'cnt' 
      ...
      % large .cnt files; "startpos" is zero => fseek won't shift "fid"
      ...
      if (bReadIntoMemory == true)
          if h.channeloffset <= 1
                ...
                %Reads the data in the selected format: 32 or 16 bits
                %[h.nchannels r.ldnsamples] = > Reads enough elements to
                %fill an m-by-n matrix, filling in elements in column order
          else
 ...
 % scaling the data to microvolts
            for i=1:h.nchannels
                bas=e(i).baseline;
                sen=e(i).senstivity;
                cal=e(i).calib;
      ...
      if eT.teeg==2
          nevents=eT.size/sizeEvent2;   %Number of events during the recording
          if nevents > 0
...
%%%% channels labels
for i=1:h.nchannels
  plab=sprintf('%c',f.electloc(i).lab); %Location of the channels (i.e. FP1, FPZ, etc.)
  if i>1 
   lab=str2mat(lab,plab);   %Saving the location of the channels in a "char" matrix
  else 
   lab=plab;  
  end  
end  

labels_electrodes = lab;    %Saving the names of the electrodes recorded;
...
%number of events
number_events = length(f.event);
events_trigger = zeros(1,number_events);
events_type = zeros(1,number_events);
for kk = 1:number_events
    
    events_trigger(1,kk) = f.event(kk).offset;
    events_type(1,kk) = f.event(kk).stimtype;
