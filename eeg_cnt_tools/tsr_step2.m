function [y,idx,mn,w]=tsr_step2(x,ref,r,wx,wref,keep,thresh)
%[y,idx,mn,w]=tsr_step2(x,ref,r,wx,wref,keep,thresh) - time-shift regression (TSPCA)
%
%  y: denoised data 
%  idx: x(idx) is aligned with y
%  mn: channel means (removed by tsr)
%  w: weights applied by tsr
% 
%  x: data to denoise (time * channels * trials)
%  ref: reference (time * channels * trials)
%  r: filter designed using tsr_step1
...
shifts=floor(size(r,1)/size(ref,2)/2);
